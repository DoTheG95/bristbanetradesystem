...
// Extract text inside parentheses from a card name, e.g. "Galemon (Limited Foil)" → "Limited Foil"
...
function isSpecialVariant(name: string): boolean {
  ...
  return (
    lower.includes('limited foil') ||
    lower.includes('alternate art') ||
    lower.includes(' sp') || lower === 'sp' ||
    lower.includes('rare pull')
  );
}
...
export default function SearchModal({ open, onClose, onAdd }: Props) {
  ...
  const [bulkSelected, setBulkSelected]   = useState<Set<string>>(new Set()); // tcgplayer_id set
  ...
  useEffect(() => {
    if (open) {
      ...
      setModalText(''); setResults([]); setSelectedCard(null); setSelectedItems([]);
      setMode('search');
  ...
  const searchPack = useCallback(async () => {
    ...
    try {
      ...
      setPackError('Failed to fetch pack. Please try again.');
    } finally {
      setLoadingPack(false);
    }
  }, [packQuery]);
  ...
  const filteredPackResults = packResults.filter(card => {
    const hasSpecialInName = isSpecialVariant(card.tcgplayer_name);
    ...
    // Special filter takes priority — these cards are identified by paren text, not rarity field
    if (specialActive && hasSpecialInName) {
      return matchesSpecialFilter(card.tcgplayer_name, specialFilters);
    }

    // Rarity filter: only apply to cards WITHOUT special variant text in name
    if (rarityActive && !hasSpecialInName) {
      return rarityFilters.has(card.rarity as Rarity);
    }
  ...
  const toggleSpecial = (s: SpecialVariant) => {
  ...
  const toggleBulkCard = (tcgId: string) => {
    setBulkSelected(prev => {
      const next = new Set(prev);
      next.has(tcgId) ? next.delete(tcgId) : next.add(tcgId);
      return next;
    });
  };
  ...
  const selectAllVisible = () => {
    setBulkSelected(prev => {
      const next = new Set(prev);
      filteredPackResults.forEach(c => { if (c.tcgplayer_id) next.add(c.tcgplayer_id); });
      return next;
    });
  };
  ...
  const deselectAllVisible = () => {
    setBulkSelected(prev => {
      const next = new Set(prev);
      filteredPackResults.forEach(c => { if (c.tcgplayer_id) next.delete(c.tcgplayer_id); });
      return next;
    });
  };
  ...
  const handleAdd = useCallback(() => {
    ...
    else if (selectedCard) onAdd(selectedCard);
    else onAdd(modalText);
    setModalText(''); setResults([]); setSelectedCard(null); setSelectedItems([]);
  }, [modalText, selectedCard, onAdd, selectedItems]);

  // ── Bulk mode: add selected ──
    ...
    >
      ...
      >
            ...
            <div style={{ display: 'flex', gap: 2, background: '#0e0e12', borderRadius: 8, padding: 3 }}>
              {(['search', 'bulk'] as Mode[]).map(m => (
                <button
                  ...
                  onClick={() => setMode(m)}
                  style={{
                    padding: '5px 16px', borderRadius: 6, border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s',
                    background: mode === m ? '#1e1e28' : 'transparent',
                    color: mode === m ? '#e8e6e0' : '#555',
                  }}
                >
              ...
              {selectedItems.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                  {selectedItems.map((it) => (
                    <div key={it.id || it.card_number} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 8px 3px 10px', background: '#1e1e32', border: '1px solid #3a3a52', borderRadius: 99, fontSize: 12, color: '#a0a0d0' }}>
                      <span style={{ maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {it.combinedName || it.tcgplayer_name || it.id}
                      </span>
                      <button type="button" onClick={(e) => { e.stopPropagation(); removeSelectedItem(it.id); }} style={{ width: 16, height: 16, borderRadius: '50%', border: 'none', background: '#2e2e42', color: '#888', cursor: 'pointer', fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>×</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ paddingBottom: 14 }}>
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && searchPack()}
                  placeholder="Pack code, e.g. BT18"
                  style={{ flex: 1, padding: '8px 12px', background: '#18181e', border: '1px solid #2a2a32', borderRadius: 8, color: '#e8e6e0', fontSize: 13, outline: 'none', fontFamily: 'inherit' }}
                />
                <button
                  onClick={searchPack}
                  disabled={loadingPack || !packQuery.trim()}
                  style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: loadingPack || !packQuery.trim() ? '#1e1e28' : '#4f46e5', color: loadingPack || !packQuery.trim() ? '#444' : '#fff', fontSize: 13, fontWeight: 600, cursor: loadingPack || !packQuery.trim() ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}
                >
                  {loadingPack ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ display: 'inline-block', width: 12, height: 12, border: '2px solid #444', borderTopColor: '#888', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
                      Loading…
                    </span>
                  ) : 'Search Pack'}
                </button>
              ...
              {/* Filters — only show when results loaded */}
                ...
                <>
                      ...
                      {RARITIES.map(r => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              padding: '3px 10px', borderRadius: 99, border: `1px solid ${active ? RARITY_COLOURS[r] : '#2a2a32'}`,
                              background: active ? `${RARITY_COLOURS[r]}22` : 'transparent',
                              color: active ? RARITY_COLOURS[r] : '#555',
                              fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.12s',
                            }}
                          >{r}</button>
                        );
                      })}
                      {rarityFilters.size > 0 && (
                        <button onClick={() => setRarityFilters(new Set())} style={{ padding: '3px 8px', borderRadius: 99, border: '1px solid #2a2a32', background: 'transparent', color: '#444', fontSize: 11, cursor: 'pointer' }}>clear</button>
                      )}
                    </div>
                  </div>

                  {/* Special variant filter */}
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                      {SPECIAL_VARIANTS.map(s => {
                        const active = specialFilters.has(s);
                        return (
                          <button
                            key={s}
                            ...
                            style={{
                              ...
                              border: `1px solid ${active ? '#a78bfa' : '#2a2a32'}`,
                      ...
                      {specialFilters.size > 0 && (
                        <button onClick={() => setSpecialFilters(new Set())} style={{ padding: '3px 8px', borderRadius: 99, border: '1px solid #2a2a32', background: 'transparent', color: '#444', fontSize: 11, cursor: 'pointer' }}>clear</button>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 4 }}>
                    <span style={{ fontSize: 11, color: '#444' }}>
                      {filteredPackResults.length} card{filteredPackResults.length !== 1 ? 's' : ''} shown
                      {bulkSelected.size > 0 && ` · ${bulkSelected.size} selected`}
                    </span>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={allVisibleSelected ? deselectAllVisible : selectAllVisible} style={{ padding: '3px 10px', borderRadius: 6, border: '1px solid #2a2a32', background: 'transparent', color: '#888', fontSize: 11, cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      {r.card_number && <div style={{ fontSize: 11, fontFamily: 'monospace', color: alreadySelected ? '#444' : '#555', marginTop: 2 }}>{r.card_number}</div>}
                    </div>
                    {alreadySelected ? <span style={{ fontSize: 11, color: '#4f46e5', flexShrink: 0 }}>added</span> : <span style={{ fontSize: 18, color: '#333', flexShrink: 0 }}>+</span>}
                  </button>
                );
              })}
            </>
          ...
          {mode === 'search' && modalText.trim().length === 0 && selectedItems.length === 0 && (
            <div style={{ padding: '28px 20px', textAlign: 'center', color: '#333', fontSize: 13 }}>Start typing to search cards</div>
          )}

          {/* Bulk mode: error */}
          {mode === 'bulk' && packError && (
            <div style={{ padding: '24px 20px', textAlign: 'center', color: '#c0392b', fontSize: 13 }}>{packError}</div>
          )}

          {/* Bulk mode: empty prompt */}
          {mode === 'bulk' && !packError && packResults.length === 0 && !loadingPack && (
            <div style={{ padding: '28px 20px', textAlign: 'center', color: '#333', fontSize: 13 }}>Enter a pack code above to load all cards from that set.</div>
          )}
          ...
          {mode === 'bulk' && filteredPackResults.length > 0 && (
            <div style={{
              ...
              {filteredPackResults.map(card => {
                const selected = bulkSelected.has(card.tcgplayer_id);
                const rColor = RARITY_COLOURS[card.rarity] ?? '#555';
                const parenText = extractParenText(card.tcgplayer_name);
                return (
                  <div
                    key={card.tcgplayer_id}
                    onClick={() => toggleBulkCard(card.tcgplayer_id)}
                    style={{
                      position: 'relative',
                      borderRadius: 8,
                      border: `2px solid ${selected ? '#4f46e5' : '#1e1e24'}`,
                    ...
                    onMouseEnter={e => { if (!selected) e.currentTarget.style.borderColor = '#2a2a3a'; }}
                    onMouseLeave={e => { if (!selected) e.currentTarget.style.borderColor = '#1e1e24'; }}
                  >
                    {/* Checkbox */}
                    <div style={{
                      position: 'absolute', top: 5, right: 5, zIndex: 2,
                      width: 16, height: 16, borderRadius: 4,
                      border: `1.5px solid ${selected ? '#4f46e5' : '#2a2a32'}`,
                      background: selected ? '#4f46e5' : 'rgba(0,0,0,0.5)',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 3, marginTop: 2 }}>
                      ...
                      {parenText && (
                        <span style={{ fontSize: 8, color: '#a78bfa', fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 60 }}>{parenText}</span>
                      )}
                    </div>
                  </div>
                );
          ...
          {/* Bulk mode: no results after filter */}
          {mode === 'bulk' && packResults.length > 0 && filteredPackResults.length === 0 && (
            <div style={{ padding: '24px 20px', textAlign: 'center', color: '#333', fontSize: 13 }}>No cards match the selected filters.</div>
          )}
        </div>
          ...
          {mode === 'search' ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setModalText(''); setResults([]); setSelectedCard(null); onClose(); }} style={{ padding: '7px 16px', borderRadius: 7, border: '1px solid #2a2a32', background: 'transparent', color: '#888', fontSize: 12, cursor: 'pointer' }}>Cancel</button>
            ...
            <>
              <span style={{ fontSize: 12, color: '#444' }}>
                {bulkSelected.size > 0 ? `${bulkSelected.size} card${bulkSelected.size > 1 ? 's' : ''} selected` : 'No cards selected'}
              </span>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{ padding: '7px 20px', borderRadius: 7, border: 'none', background: bulkSelected.size === 0 ? '#1e1e28' : '#4f46e5', color: bulkSelected.size === 0 ? '#444' : '#fff', fontSize: 12, fontWeight: 600, cursor: bulkSelected.size === 0 ? 'not-allowed' : 'pointer' }}
                >
